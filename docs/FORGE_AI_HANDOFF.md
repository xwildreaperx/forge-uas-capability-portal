...
Unit Administration Part 2 provides a per-administered-Unit stewardship projection. It derives led and supported Projects, current and historical work, Problem coverage, assistance, Lessons, responsibility, maturity/outcome summaries, and meaningful Project/administrative Activity from existing relational records. The Unit selector changes stewardship scope only; broad discovery remains intact and server mutations remain narrowly authorized. “Problems Addressed” never implies that a Unit experiences a capability gap. No Unit or user ranking, success rate, or manually maintained Unit report is introduced.
