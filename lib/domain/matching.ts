...
export function scoreProblemMatch(
  ...
  const categoryScore =
    input.category && input.category === candidate.category ? 0.2 : 0;
  const sharedTags = (input.tags ?? []).filter((tag) =>
    candidate.tags.includes(tag),
  ).length;
  return Math.min(
    1,
    titleScore * 0.75 + categoryScore + Math.min(0.2, sharedTags * 0.1),
  );
}
...
export function findRelatedProblems(
  ...
  return candidates
    .map((problem) => ({
      ...problem,
      score: scoreProblemMatch(input, problem),
    }))
